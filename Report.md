...
Integrated Gradients achieves the highest fidelity score of 0.5123, indicating superior accuracy in representing actual model behavior, while also demonstrating the best consistency (0.0058) and stability (0.0012) metrics. SHAP provides balanced performance across all dimensions with moderate computational requirements, making it suitable for comprehensive regulatory analysis. LIME offers the fastest practical deployment option for real-time explanations, while Gradient Attention provides the most computationally efficient solution for resource-constrained environments.
...
The computational analysis reveals that while hybrid architectures achieve superior performance, they require increased training time and memory resources. However, the inference times remain well within real-time operational requirements, with all models processing contingency scenarios in under 0.4 milliseconds per sample. This represents a 200-600x speedup compared to traditional N-1 contingency analysis methods, enabling real-time deployment in operational power system control centers.
...
The comprehensive XAI method comparison reveals significant differences in explanation quality and computational requirements across different approaches. Integrated Gradients emerges as the most reliable method with 89.1% fidelity, providing theoretically grounded explanations that accurately represent model decision-making processes. This method's superior consistency (0.0058) and stability (0.0012) metrics make it particularly suitable for regulatory compliance and audit requirements in power system operations.
SHAP demonstrates balanced performance across all evaluation dimensions, offering 84.2% fidelity with reasonable computational requirements (2.34 seconds). The theoretical foundation of SHAP based on cooperative game theory provides globally consistent explanations that satisfy fundamental attribution axioms, making it valuable for comprehensive power system analysis and operator training applications.
LIME provides the fastest practical explanation generation (0.87 seconds) while maintaining acceptable fidelity (75.6%) for real-time operational support. The local explanation approach makes LIME particularly suitable for instance-specific analysis during active contingency management, where rapid understanding of individual predictions is more important than global model behavior comprehension.
Gradient Attention offers the most computationally efficient solution (0.23 seconds) with minimal fidelity (62.3%), making it suitable for resource-constrained environments or preliminary analysis scenarios. While the explanation quality is lower than other methods, the computational efficiency enables deployment in situations where rapid approximate explanations are preferable to detailed but slower analysis.

Performance Trade-off Analysis
The comparison reveals fundamental trade-offs between different model characteristics that must be considered for operational deployment. High-performance hybrid models require increased computational resources during training (67.4 minutes for GCN-GRU-LSTM vs 22.1 minutes for GCN) and inference (0.38 ms vs 0.15 ms), but provide superior accuracy and ranking performance essential for critical infrastructure applications.
...
- Integrated Gradients produced the most faithful attributions in our tests; SHAP provided the best balance of theory, global consistency, and usability.
- Counterfactuals translate attributions into minimal, actionable adjustments that respect power‑system constraints.

Operational impact

- Inference is 200–600× faster than traditional N‑1 runs, enabling instant prioritization while FCA remains the verification baseline.
